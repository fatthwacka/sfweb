...
import { ArrowLeft, Sparkles, Loader2, Download, Info, Eye, Brain, CheckSquare, Square, Upload, Package, User, Cat, Palette, X, Image, Mountain, Code, FileCode, Settings2, ChevronDown, ChevronUp, MessageSquare, Cpu, Type, Briefcase, Sliders, FlaskConical, Image as ImageIcon } from 'lucide-react';
...
export default function AIImageGenerator() {
  ...
  // Extended to include brand asset metadata for intelligent prompt enhancement
  ...
  const [committedPrompt, setCommittedPrompt] = useState(''); // The saved version
  ...
  // Enhancement Guidelines state (Path A: no assets, Path B: with assets)
  ...
  // Panel collapse state (true = expanded, false = collapsed)
  // Prompt-related panels start expanded, others start collapsed
  const [panelStates, setPanelStates] = useState({
    imageDescription: true,      // expanded
    aiModel: false,              // collapsed
    textOverlays: false,         // collapsed
    brandIntelligence: false,    // collapsed
    styleSettings: false,        // collapsed
    enhancementGuidelines: true, // expanded
    imageIngredients: false,     // collapsed
    generatedPreview: false,     // collapsed
    systemPrompt: true,          // expanded
    apiPreview: true,            // expanded
  });
  ...
  // Load system prompt on mount (hybrid: localStorage draft → Supabase committed → default)
  useEffect(() => {
    const loadSystemPrompt = async () => {
      ...
      try {
        ...
        // 2. Fetch committed prompt from Supabase
        ...
        // 3. Use draft if different from committed, otherwise use committed
        if (localDraft && localDraft !== committedText) {
          ...
          // Clear stale draft if it matches committed
          if (localDraft) {
        ...
        // Fall back to localStorage draft if API fails
        const localDraft = localStorage.getItem(LOCALSTORAGE_KEY);
        if (localDraft) {
          setSystemPrompt(localDraft);
        }
  ...
  const saveSystemPrompt = async () => {
    ...
    try {
      ...
      toast({
        title: 'Save failed',
        description: 'Could not save the system prompt. Please try again.',
        variant: 'destructive'
      });
    } finally {
  ...
  // Reset prompt to committed version
  ...
  // Auto-save enhancement drafts to localStorage (debounced)
  useEffect(() => {
  ...
  // Save enhancement guideline to Supabase
  ...
  // Reset enhancement to committed version
  ...
  // Build API preview payload (dry run)
  ...
  // Handle brand asset selection from picker
  const handleBrandAssetSelect = async (asset: any, imageDataUrl: string, clientInfo?: { id: string; industry?: string }) => {
    ...
    try {
      // Convert data URL to File object for compatibility with existing system
      const response = await fetch(imageDataUrl);
      const blob = await response.blob();
      const file = new File([blob], `${asset.name}.${blob.type.split('/')[1] || 'png'}`, { type: blob.type });

      // Process the image (compression for model/scene, raw for products)
      const preview = URL.createObjectURL(file);
      let base64: string;
      let mimeType: string = file.type;
      ...
      // Set ingredient WITH brandAssetMetadata in a single state update (avoids race condition)
      ...
      setIngredients(prev => ({
        ...
        [brandAssetTargetSlot]: {
          ...
          // Store comprehensive brand asset metadata for prompt enhancement
          brandAssetMetadata: {
            assetId: asset.id,
      ...
      console.log('🏷️ Brand asset metadata stored:', {
        assetName: asset.name,
        material: asset.material,
        similarTo: asset.similar_to,
        usageNotes: asset.usage_notes,
      ...
      toast({
        ...
        description: `"${asset.name}" loaded into ${slot?.label || brandAssetTargetSlot}. Use ${slot?.tag} in your prompt.`,
      });
    } catch (error) {
  ...
  // Open brand asset picker for a specific slot
  ...
  const enhancePrompt = async () => {
    ...
    try {
      // Get active ingredient info for context - INCLUDING brand asset metadata
      const activeIngredients = INGREDIENT_SLOTS
        .filter(slot => isSlotActive(slot.id) && ingredients[slot.id])
        .map(slot => {
    ...
    return INGREDIENT_SLOTS
      ...
      .map(slot => {
        ...
        return {
          ...
          mimeType: ingredient.mimeType,  // Use stored mimeType (correct after compression)
          description: ingredient.description,
          // Include brand asset metadata if available (for intelligent prompt enhancement)
          brandAssetMetadata: ingredient.brandAssetMetadata || undefined,
        };
      });
  };
    ...
    try {
      const response = await fetch('/api/ai/generate-image', {
        ...
        body: JSON.stringify({
          ...
          // Brand intelligence is now handled via asset metadata (brandAssetMetadata)
          // The Brand Intelligence panel UI is kept for visual reference but disconnected from prompt path
          // All brand context (industry, material, usage notes) flows through the asset attachment path
          // brandIntelligence: undefined, // Intentionally disabled - use asset metadata instead
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold text-salmon">Configuration</h2>
                    ...
                    {/* Image Description Section - MOVED TO TOP */}
                    ...
                    {/* AI Model Section - Collapsible & Compact */}
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent className="max-w-xs">
                              <div className="space-y-1 text-xs">
                                <p><strong>Nano Banana:</strong> Multimodal AI with K-scale resolutions</p>
                                <p><strong>Imagen:</strong> Text-only with px resolutions</p>
                              </div>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                              ...
                              if (isDisabled) {
                                return (
                                  <div
                                    ...
                                    className="radio-btn-base opacity-40 cursor-not-allowed bg-gray-800/50 border-gray-600/50 py-2"
                                  >
                                    <div className="space-y-0.5 text-center">
                                      <div className="radio-btn-text text-gray-500 text-xs">{modelOption.label}</div>
                                    </div>
                                  </div>
                                );
                              }

                              // Compact description for Imagen Ultra
                              const compactDesc = modelOption.value === 'imagen-4.0-ultra-generate-001'
                                ? '2048px ($0.04)'
                                : modelOption.nativeResolutionFormat === 'k-scale'
                                  ? `${modelOption.nativeResolution} ${modelOption.description.match(/\(\$[\d\.]+\)/)?.[0] || ''}`
                                  : `${modelOption.nativeResolution} ${modelOption.description.match(/\(\$[\d\.]+\)/)?.[0] || ''}`;

                              return (
                                <label
                                  key={modelOption.value}
                                  className={`radio-btn-base py-2 ${
                                ...
                                >
                                  ...
                                  />
                                  <div className="space-y-0.5 text-center">
                                    <div className="radio-btn-text text-xs">{modelOption.label}</div>
                                    <div className="radio-btn-desc text-[10px]">{compactDesc}</div>
                                  </div>
                                </label>
                              );
                            })}
                    ...
                    {/* Text Overlays Section - Collapsible */}
                      ...
                      >
                        <div className="flex items-center space-x-2">
                          ...
                          {(includeTitle || includeSubtitle) && (
                            <span className="text-xs text-gray-500">({includeTitle ? 'Title' : ''}{includeTitle && includeSubtitle ? ' + ' : ''}{includeSubtitle ? 'Subtitle' : ''})</span>
                          )}
                        </div>
                      </div>
                    ...
                    {/* Brand Intelligence Section - Collapsible */}
                    <div className="bg-gradient-to-br from-violet-900/30 via-gray-800 to-gray-900 border border-violet-500/30 rounded-lg overflow-hidden">
                      <div
                        className="flex items-center justify-between p-3 cursor-pointer hover:bg-violet-800/20 transition-colors"
                        onClick={() => togglePanel('brandIntelligence')}
                      >
                        <div className="flex items-center space-x-2">
                          <Briefcase className="h-4 w-4 text-violet-400" />
                          <h3 className="text-sm font-semibold text-violet-300">Brand Intelligence</h3>
                          {selectedBrand && (
                            <span className="text-xs text-violet-400/70">(Active)</span>
                          )}
                        </div>
                      </div>
                      {panelStates.brandIntelligence && (
                        <div className="px-4 pb-4 space-y-3">
                    ...
                    {/* Style & Settings Section - Collapsible */}
                    <div className="bg-gradient-to-br from-slate-800/30 via-gray-800 to-gray-900 border border-slate-500/30 rounded-lg overflow-hidden">
                      <div
                      ...
                      >
                        <div className="flex items-center space-x-2">
                          ...
                          <h3 className="text-sm font-semibold text-slate-300">Style & Settings</h3>
                        </div>
                      </div>
                    ...
                    {/* Enhancement Guidelines Section - Collapsible */}
                    <div className="bg-gradient-to-br from-indigo-900/30 via-gray-800 to-gray-900 border border-indigo-500/30 rounded-lg overflow-hidden">
                      <div
                        className="flex items-center justify-between p-3 cursor-pointer hover:bg-indigo-800/20 transition-colors"
                        onClick={() => togglePanel('enhancementGuidelines')}
                      >
                        <div className="flex items-center space-x-2">
                          <Settings2 className="h-4 w-4 text-indigo-400" />
                          <h3 className="text-sm font-semibold text-indigo-300">Enhancement Guidelines</h3>
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent className="max-w-sm">
                              <div className="space-y-2 text-xs">
                                <p><strong>Enhancement Guidelines:</strong> Instructions for Gemini when elaborating your prompts.</p>
                                <p><strong>Path A:</strong> Used when no reference images are uploaded.</p>
                                <p><strong>Path B:</strong> Used when you have uploaded image ingredients.</p>
                              </div>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </div>
                      {panelStates.enhancementGuidelines && (
                      <div className="px-4 pb-4 space-y-3">
                      {/* Path Tabs */}
                      <div className="flex space-x-1 bg-gray-800/50 rounded-lg p-1">
                        <button
                          onClick={() => setActiveEnhancementTab('no_assets')}
                          className={`flex-1 text-xs px-3 py-1.5 rounded-md transition-all ${
                            activeEnhancementTab === 'no_assets'
                        ...
                        <button
                          onClick={() => setActiveEnhancementTab('with_assets')}
                          className={`flex-1 text-xs px-3 py-1.5 rounded-md transition-all ${
                            activeEnhancementTab === 'with_assets'
                      ...
                      {isLoadingEnhancements ? (
                        <div className="flex items-center justify-center py-6">
                          ...
                          <span className="ml-2 text-xs text-gray-400">Loading guidelines...</span>
                        </div>
                      ) : (
                        <>
                          {activeEnhancementTab === 'no_assets' ? (
                            <div className="space-y-2">
                              <Textarea
                                value={enhancementNoAssets}
                                onChange={(e) => setEnhancementNoAssets(e.target.value)}
                                placeholder="Instructions for enhancing prompts without reference images..."
                                className="min-h-[400px] max-h-[800px] resize-y bg-gray-900/50 border-gray-600 text-gray-200 text-[10px] font-mono leading-relaxed"
                              />
                              <div className="flex items-center justify-between">
                                <span className="text-xs text-gray-500">{enhancementNoAssets.length} chars</span>
                                <div className="flex gap-2">
                                  {enhancementNoAssets !== committedEnhancementNoAssets && (
                                    <Button
                                      onClick={() => resetEnhancementToCommitted('enhancement_no_assets')}
                                      variant="ghost"
                                      size="sm"
                                  ...
                                  <Button
                                    onClick={() => saveEnhancementGuideline('enhancement_no_assets')}
                                    disabled={isSavingEnhancement || enhancementNoAssets === committedEnhancementNoAssets}
                                    variant="outline"
                                    size="sm"
                                  ...
                                  >
                                    {isSavingEnhancement ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Save'}
                                  </Button>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="space-y-2">
                              <Textarea
                                value={enhancementWithAssets}
                                onChange={(e) => setEnhancementWithAssets(e.target.value)}
                                placeholder="Instructions for enhancing prompts WITH reference images..."
                                className="min-h-[400px] max-h-[800px] resize-y bg-gray-900/50 border-gray-600 text-gray-200 text-[10px] font-mono leading-relaxed"
                              />
                              <div className="flex items-center justify-between">
                                <span className="text-xs text-gray-500">{enhancementWithAssets.length} chars</span>
                                <div className="flex gap-2">
                                  {enhancementWithAssets !== committedEnhancementWithAssets && (
                                    <Button
                                      onClick={() => resetEnhancementToCommitted('enhancement_with_assets')}
                                      variant="ghost"
                                      size="sm"
                                      className="h-7 text-xs text-gray-400 hover:text-white"
                                    >
                                  ...
                                  <Button
                                    onClick={() => saveEnhancementGuideline('enhancement_with_assets')}
                                    disabled={isSavingEnhancement || enhancementWithAssets === committedEnhancementWithAssets}
                                    variant="outline"
                                    size="sm"
                                    className={`h-7 text-xs ${
                                      enhancementWithAssets !== committedEnhancementWithAssets
                  ...
                  <div className="space-y-6">
                    <h2 className="text-2xl font-semibold text-salmon">Preview & Ingredients</h2>

                    {/* Image Ingredients Section - MOVED TO RIGHT COLUMN - Collapsible */}
                    <div className={`rounded-lg border overflow-hidden transition-all ${
                      supportsReferenceImages()
                        ? 'bg-gradient-to-br from-purple-900/30 via-gray-800 to-gray-900 border border-purple-500/30'
                        : 'bg-gray-800/50 border-gray-600/50'
                    }`}>
                      {/* Collapsible Header */}
                      <div
                        className={`flex items-center justify-between p-3 cursor-pointer transition-colors ${supportsReferenceImages() ? 'hover:bg-purple-800/20' : 'hover:bg-gray-700/30'}`}
                        onClick={() => togglePanel('imageIngredients')}
                      >
                        <div className="flex items-center space-x-2">
                          <FlaskConical className={`h-4 w-4 ${supportsReferenceImages() ? 'text-purple-400' : 'text-gray-500'}`} />
                      ...
                      {/* Quick Actions Bar */}
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-purple-400">Drag images or fetch from library</span>
                        <button
                          onClick={() => openBrandAssetPicker('product1')}
                          className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-600/20 hover:bg-amber-600/30 border border-amber-500/40 rounded text-xs text-amber-300 transition-colors"
                        >
                              ...
                              <div key={slot.id} className="relative">
                                ...
                                >
                                  {ingredient ? (
                                    ...
                                    <div className="aspect-square flex flex-col items-center justify-center p-2 relative group/slot">
                                      ...
                                      {/* Brand Assets quick button - appears on hover */}
                                      {isActive && (
                                        <button
                                          onClick={(e) => {
                                            e.stopPropagation();
                                          ...
                                          className="absolute bottom-1 right-1 p-1 rounded bg-amber-600/60 hover:bg-amber-600 text-white opacity-0 group-hover/slot:opacity-100 transition-opacity"
                              ...
                              <div key={slot.id} className="relative">
                                ...
                                >
                                  {ingredient ? (
                                    ...
                                    <div className="aspect-video flex flex-col items-center justify-center p-3 relative group/slot">
                                      ...
                                      {isActive && (
                                        <button
                                          ...
                                          className="absolute bottom-1 right-1 p-1.5 rounded bg-amber-600/60 hover:bg-amber-600 text-white opacity-0 group-hover/slot:opacity-100 transition-opacity"
                                          title="Browse brand assets"
                                        >
                                          <Package className="h-3 w-3" />
                                        </button>
                      ...
                      {hasIngredients() && (
                        <div className="text-xs text-purple-400 bg-purple-900/30 rounded p-2">
                          ...
                          {getActiveIngredientTags().map((tag) => (
                    ...
                    {/* Generated Image Preview - Collapsible */}
                    <div className="bg-gradient-to-br from-orange-900/30 via-gray-800 to-gray-900 border border-orange-500/30 rounded-lg overflow-hidden">
                      <div
                        className="flex items-center justify-between p-3 cursor-pointer hover:bg-orange-800/20 transition-colors"
                        onClick={() => togglePanel('generatedPreview')}
                      >
                        <div className="flex items-center space-x-2">
                      ...
                      {panelStates.generatedPreview && (
                      <div className="px-4 pb-4 space-y-4">
                    <div className="bg-gradient-to-br from-gray-800 to-purple-900/50 rounded-lg min-h-[300px] flex items-center justify-center border-2 border-dashed border-purple-500/30">
                      {generatedImage ? (
                        ...
                        <div className="text-center text-gray-400">
                          ...
                          <p className="text-lg font-medium text-gray-300">Generated image will appear here</p>
                          <p className="text-sm text-gray-500">Enter a description and click "Generate AI Image" to start</p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-cyan-600/50 rounded-lg overflow-hidden">
                      <div
                      ...
                      >
                        <div className="flex items-center space-x-2">
                          ...
                          <h3 className="text-sm font-semibold text-cyan-300">System Prompt</h3>
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            ...
                            <TooltipContent className="max-w-sm">
                              <div className="space-y-2 text-xs">
                                <p><strong>System Prompt:</strong> This guides how Gemini enhances your image descriptions.</p>
                                <p>Edit inline to test changes. Press "Commit & Save" when happy.</p>
                                <p className="text-amber-400">⚠️ Vertex AI rules are preserved - the enhancer adds style/mood guidance.</p>
                              </div>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      ...
                      {isLoadingPrompt ? (
                        <div className="flex items-center justify-center py-8">
                          ...
                          <span className="ml-2 text-sm text-gray-400">Loading prompt...</span>
                        </div>
                      ) : (
                        <>
                          <Textarea
                          ...
                          <div className="flex items-center justify-between">
                            <div className="text-xs text-gray-500">
                              ...
                              {promptHasUnsavedChanges && (
                                <span className="ml-2 text-amber-500">• Draft auto-saved locally</span>
                              )}
                            </div>
                            <div className="flex gap-2">
                              {promptHasUnsavedChanges && (
                              ...
                              <Button
                                ...
                                variant="outline"
                                size="sm"
                                className={promptHasUnsavedChanges
                    ...
                    {/* API Preview Window - Collapsible */}
                    <div className="bg-gradient-to-br from-emerald-900/30 via-gray-800 to-gray-900 border border-emerald-500/30 rounded-lg overflow-hidden">
                      <div
                        className="flex items-center justify-between p-3 cursor-pointer hover:bg-gray-600/50 transition-colors"
                      ...
                      >
                        <div className="flex items-center space-x-2">
                          ...
                          <h3 className="text-sm font-semibold text-emerald-300">Vertex API Preview</h3>
                          <span className="text-xs px-2 py-0.5 bg-gray-700 text-gray-400 rounded-full">
                            Read-only
                          </span>
                        </div>
                      </div>
                      {panelStates.apiPreview && (
                      <div className="px-4 pb-4 space-y-3">
                        <div className="flex items-center gap-2">
                          <Button
                            onClick={(e) => { e.stopPropagation(); buildApiPreview(); }}
                            disabled={isLoadingPreview || !prompt.trim()}
                            variant="outline"
                            size="sm"
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center justify-between text-xs">
                            <span className="px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded flex items-center gap-1">
                              {apiPreview.apiType === 'gemini' ? (
                                <><Sparkles className="h-3 w-3" /> Nano Banana (Gemini)</>
                              ) : (
                                <><ImageIcon className="h-3 w-3" /> Imagen</>
                              )}
                            </span>
                            <code className="text-gray-500 truncate max-w-[250px]" title={apiPreview.endpoint}>
                              {apiPreview.endpoint?.split('/').slice(-2).join('/')}
                            </code>
                          </div>

                          {/* Enhanced Prompt Preview */}
                          {apiPreview.enhancedPrompt && (
                            <div className="space-y-1">
                              <Label className="text-xs text-emerald-400">Enhanced Prompt</Label>
                              <div className="p-2 bg-gray-800/50 rounded border border-gray-700 max-h-[100px] overflow-y-auto">
                          ...
                          {/* Request Body JSON */}
                          <div className="space-y-1">
                            <Label className="text-xs text-emerald-400">Request Body</Label>
                            <pre className="p-3 bg-gray-950 rounded border border-gray-700 text-xs text-gray-300 overflow-x-auto max-h-[250px] overflow-y-auto font-mono">
                              {JSON.stringify(apiPreview.requestBody, null, 2)}
                            </pre>
                          </div>
                          ...
                          <div className="flex items-center justify-between text-xs text-gray-500 border-t border-gray-700 pt-2">
                            <span>
                              Model: <code className="text-gray-400">{model}</code>
                            </span>
                            <span>
                              {aspectRatio} • {resolution}
                            </span>
                            <span>
                              {getActiveIngredients().length > 0
                                ? `${getActiveIngredients().length} ingredient(s)`
                                : 'No ingredients'
                              }
                            </span>
                          </div>
                      ...
                      {!apiPreview && (
                        <div className="text-center py-4 text-gray-500">
                          ...
                          <p className="text-xs">Click "Build Preview" to see the API payload</p>
                          <p className="text-xs text-gray-600">No API calls will be made - this is a dry run</p>
                        </div>
                      )}
                      </div>
                      )}
