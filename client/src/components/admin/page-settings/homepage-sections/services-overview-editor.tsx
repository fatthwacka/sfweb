...
export function ServicesOverviewEditor({ onChange, onSave }: ServicesOverviewEditorProps) {
  ...
  const [servicesSettings, setServicesSettings] = useState({
    ...
    seoDescription: 'Professional photography and videography services in Durban'
  });
  ...
  const handleImageSelect = (type: 'photography' | 'videography', imagePath: string) => {
    setServicesSettings(prev => ({
      ...prev,
      [`${type}Image`]: imagePath
    }));
    
    onChange({ [`${type}Image`]: imagePath });
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Photography Image Browser Dialog */}
      <Dialog open={isPhotoBrowserOpen} onOpenChange={setIsPhotoBrowserOpen}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Folder className="w-5 h-5" />
              Browse Site Images - Photography
            </DialogTitle>
          </DialogHeader>
                    ...
                    <div className="grid grid-cols-6 md:grid-cols-8 gap-3">
                      {(images as string[]).map((imagePath, index) => (
                        <div
                          ...
                          onClick={() => handleImageSelect('photography', imagePath)}
                        >
                          <img 
                            src={imagePath} 
      ...
      {/* Videography Image Browser Dialog */}
      <Dialog open={isVideoBrowserOpen} onOpenChange={setIsVideoBrowserOpen}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Folder className="w-5 h-5" />
              Browse Site Images - Videography
            </DialogTitle>
          </DialogHeader>
                    ...
                    <div className="grid grid-cols-6 md:grid-cols-8 gap-3">
                      {(images as string[]).map((imagePath, index) => (
                        <div
                          key={`${folder}-${index}`}
                          className="group relative cursor-pointer rounded-lg overflow-hidden border border-border hover:border-primary transition-all duration-200 hover:scale-105"
                          onClick={() => handleImageSelect('videography', imagePath)}
                        >
                          <img 
                            src={imagePath} 
                            alt={imagePath.split('/').pop() || 'Image'}
